...
import com.socialcomputing.corporama.utils.HashUtil;
...
public class RestProvider {
    ...
    public static final String CALLBACK_URL = "http://corporama.wps.cloudbees.net/";
    ...
    String build(String query, String token) {
        ...
        try {
            ...
            UrlHelper corporamaClient = new UrlHelper(POST, RestProvider.BK_URL + "/api/v3/post/_search");
            corporamaClient.addParameter("q", q.toString());
            corporamaClient.addParameter("oauth_token", token);
            corporamaClient.openConnections();
            
            JsonNode response = mapper.readTree(corporamaClient.getStream());
            ...
            corporamaClient.closeConnections();
        }
        catch (Exception e) {
     ...
     * code returned by corporama after authentication
     * 
